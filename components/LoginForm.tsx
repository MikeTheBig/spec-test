...
export default function LoginForm({ onClose } : { onClose: ()=>void }){
  ...
  async function submit(e:any){
    ...
    setLoading(true)
    try {
      const res = await fetch(`${API}/api/auth/login`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ email, passwordHash: password }) })
      let j: any = null
  ...
  return (
    <Modal onClose={onClose}>
      <h2 className="text-xl font-semibold mb-4">Log ind</h2>
      <form onSubmit={submit} className="space-y-3">
        ...
        <div className="flex justify-end">
          ...
          <button type="submit" className="px-3 py-1 bg-slate-700 text-white">{loading ? '...' : 'Login'}</button>
        </div>
      </form>
    </Modal>
  )
